...
//get cartesian elements from keplerian elements defined relative to an object obs
    double a = m.a*1e3;
    double e = m.e;
    //1: setting M(t)
    double M;
    ...
    m.x = x*(cos(w)*cos(Om)-sin(w)*cos(i)*sin(Om)) - y*(sin(w)*cos(Om)+cos(w)*cos(i)*sin(Om)) + obs.x;
    m.y = x*(cos(w)*sin(Om)+sin(w)*cos(i)*cos(Om)) + y*(cos(w)*cos(i)*cos(Om)-sin(w)*sin(Om)) + obs.y;
    m.z = x*(sin(w)*sin(i)) + y*(cos(w)*sin(i)) + obs.z;
    m.vx = vx*(cos(w)*cos(Om)-sin(w)*cos(i)*sin(Om)) - vy*(sin(w)*cos(Om)+cos(w)*cos(i)*sin(Om)) + obs.vx;
    m.vy = vx*(cos(w)*sin(Om)+sin(w)*cos(i)*cos(Om)) + vy*(cos(w)*cos(i)*cos(Om)-sin(w)*sin(Om)) + obs.vy;
    m.vz = vx*(sin(w)*sin(i)) + vy*(cos(w)*sin(i)) + obs.vz;
...
void write_hamiltonian(vector<object> &bodies,double t,ofstream& file){
    //Writes Hamiltonian in the given file
